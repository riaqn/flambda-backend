...
module Lattices_mono = struct
  ...
  let rec apply' : type a b d. b obj -> (a, b, d) morph -> (a -> b) * int =
   fun dst -> function
    ...
    | Set (sax, f) ->
      Product.set sax (apply (proj_obj (Product.dst sax) dst) f), 1
...
let unique_to_linear m =
  S.Neg_Pos.(apply Linearity.Obj.obj_s (lift C.Unique_to_linear) m)

let linear_to_unique m =
  S.Pos_Neg.(apply Uniqueness.Obj.obj_s (lift C.Linear_to_unique) m)

let regional_to_local m =
  S.Pos_Pos.(apply Locality.Obj.obj_s (lift C.Regional_to_local) m)

let locality_as_regionality m =
  S.Pos_Pos.(apply Regionality.Obj.obj_s (lift C.Locality_as_regionality) m)

let regional_to_global m =
  S.Pos_Pos.(apply Locality.Obj.obj_s (lift C.Regional_to_global) m)
...
module Comonadic_with_regionality = struct
  ...
  let regionality m =
    S.Pos_Pos.(apply Regionality.Obj.obj_s (lift (C.Proj (Obj.obj, Axis0))) m)

  let min_with_regionality m =
    S.Pos_Pos.(apply Obj.obj_s (lift (C.Min_with Axis0)) (S.disallow_right m))

  let max_with_regionality m =
    S.Pos_Pos.(apply Obj.obj_s (lift (C.Max_with Axis0)) (S.disallow_left m))

  let set_regionality_max m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Const_max Regionality)))
        (S.disallow_left m))

  let set_regionality_min m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Const_min Regionality)))
        (S.disallow_right m))

  let linearity m =
    S.Pos_Pos.(apply Linearity.Obj.obj_s (lift (C.Proj (Obj.obj, Axis1))) m)

  let min_with_linearity m =
    S.Pos_Pos.(apply Obj.obj_s (lift (C.Min_with Axis1)) (S.disallow_right m))

  let max_with_linearity m =
    S.Pos_Pos.(apply Obj.obj_s (lift (C.Max_with Axis1)) (S.disallow_left m))

  let set_linearity_max m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis1, C.Const_max Linearity)))
        (S.disallow_left m))

  let set_linearity_min m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis1, C.Const_min Linearity)))
        (S.disallow_right m))
...
module Comonadic_with_locality = struct
  ...
  let locality m =
    S.Pos_Pos.(apply Locality.Obj.obj_s (lift (C.Proj (Obj.obj, Axis0))) m)
  ...
  let set_locality_max m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Const_max Locality)))
        (S.disallow_left m))

  let set_locality_min m =
    S.Pos_Pos.(
      apply Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Const_min Locality)))
        (S.disallow_right m))
...
let alloc_as_value m =
  ...
  let comonadic =
    S.Pos_Pos.(
      apply Value.Comonadic.Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Locality_as_regionality)))
        comonadic)
  in
  { comonadic; monadic }
...
let alloc_to_value_l2r m =
  ...
  let comonadic =
    S.Pos_Pos.(
      apply Value.Comonadic.Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Local_to_regional)))
        comonadic)
  in
  { comonadic; monadic }
...
let value_to_alloc_r2g : type l r. (l * r) Value.t -> (l * r) Alloc.t =
 fun m ->
  ...
  let comonadic =
    S.Pos_Pos.(
      apply Alloc.Comonadic.Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Regional_to_global)))
        comonadic)
  in
  { comonadic; monadic }
...
let value_to_alloc_r2l m =
  ...
  let comonadic =
    S.Pos_Pos.(
      apply Alloc.Comonadic.Obj.obj_s
        (lift (C.Set (Product.SAxis0, C.Regional_to_local)))
        comonadic)
  in
  { comonadic; monadic }
