...
module Lattices = struct
  ...
  module Portability = struct
    ...
    include Total (struct
      ...
      let legacy = Unportable

      let le a b =
        match a, b with
        | Portable, _ | _, Unportable -> true
        | Unportable, Portable -> false

      let join a b =
        match a, b with
        | Unportable, _ | _, Unportable -> Unportable
        | Portable, Portable -> Portable

      let meet a b =
        match a, b with
        | Portable, _ | _, Portable -> Portable
        | Unportable, Unportable -> Unportable

      let print ppf = function
        | Portable -> Format.fprintf ppf "portable"
        | Unportable -> Format.fprintf ppf "unportable"
    end)
  end
...
module Lattices_mono = struct
  ...
  let portable_to_contended = function
    ...
    | Portability.Unportable -> Contention.Uncontended

  let contended_to_portable = function
    | Contention.Contended -> Portability.Portable
    | Contention.Uncontended -> Portability.Unportable
...
module Comonadic_with_regionality = struct
  ...
  let submode_log m0 m1 ~log : _ result =
    match submode_log m0 m1 ~log with
    | Ok () -> Ok ()
    | Error { left = reg0, lin0, syn0; right = reg1, lin1, syn1 } ->
...
module Comonadic_with_locality = struct
  ...
  let zap_to_legacy m =
    ...
    locality, linearity, portability
  ...
  let submode_log m0 m1 ~log : _ result =
    match submode_log m0 m1 ~log with
    | Ok () -> Ok ()
    | Error { left = loc0, lin0, syn0; right = loc1, lin1, syn1 } ->
  ...
  let check_const m =
    ...
    let portability = Portability.check_const (proj Portability m) in
    locality, linearity, portability
end
...
module Monadic = struct
  ...
  let submode_log m0 m1 ~log : _ result =
    match submode_log m0 m1 ~log with
    | Ok () -> Ok ()
    | Error { left = uni0, con0; right = uni1, con1 } ->
      if Uniqueness.Const.le uni0 uni1
      then
        if Contention.Const.le con0 con1
        then assert false
        else Error (Error (Contention, { left = con0; right = con1 }))
  ...
  let check_const m =
    ...
    let contention = Contention.check_const (proj Contention m) in
    uniqueness, contention
end
...
module Value = struct
  ...
  let split { regionality; linearity; portability; uniqueness; contention } =
    let monadic = uniqueness, contention in
    let comonadic = regionality, linearity, portability in
    { comonadic; monadic }

  let merge { comonadic; monadic } =
    let regionality, linearity, portability = comonadic in
    let uniqueness, contention = monadic in
    { regionality; linearity; portability; uniqueness; contention }
...
module Alloc = struct
  ...
  let split { locality; linearity; portability; uniqueness; contention } =
    let monadic = uniqueness, contention in
    let comonadic = locality, linearity, portability in
    { comonadic; monadic }

  let merge { comonadic; monadic } =
    let locality, linearity, portability = comonadic in
    let uniqueness, contention = monadic in
    { locality; linearity; portability; uniqueness; contention }
  ...
  let of_const { locality; linearity; portability; uniqueness; contention } =
    let comonadic = Comonadic.of_const (locality, linearity, portability) in
    let monadic = Monadic.of_const (uniqueness, contention) in
    { comonadic; monadic }
