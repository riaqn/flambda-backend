...
let apply_mode_annots ~loc ~env ~ty_expected (locality, linearity, uniqueness) mode =
  ...
  Option.iter (fun locality ->
    match Locality.equate (Locality.of_const locality) (Alloc.areality mode) with
    | Ok () -> ()
...
and type_argument ?explanation ?recarg env (mode : expected_mode) sarg
  ...
  match may_coerce with
    Some (safe_expect, lv) ->
      ...
      Value.submode_exn eta_mode (Value.max_with_areality Regionality.regional);
